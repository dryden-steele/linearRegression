...
// Will probably tweak this somehow to get more accurate without destrying our
// efficency
const int ORDER = 100;
const int CSV_NUM = 40;
const int days_in_month[] = {31,  59,  90,  120, 151, 181,
                             212, 243, 273, 303, 333};
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < num_processes; i++) {
        ...
        if (i == 0) {
            ...
            data_displacements[i] =
                data_displacements[i - 1] + data_sizes[i - 1];
        }
    ...
    if (rank == 0) {
        ...
        for (int i = 1; i < num_processes; i++) {
            for (int j = 0; j < data_sizes[i]; j++) {
                ...
                // Send filename
                // printf("Sending filename \"%s\" to process %d from process
                // %d\n", csv_files[index], i, rank); fflush(stdout);
                MPI_Send(&csv_files[index], strlen(csv_files[index]), MPI_CHAR,
                         i, 0, MPI_COMM_WORLD);

                // Send file size
                // printf("Sending file length %d to process %d from process
                // %d\n", csv_lines[index], i, rank); fflush(stdout);
        ...
        for (int i = 0; i < data_sizes[rank]; i++) {
            ...
            // Receive filename
            MPI_Recv(&local_csv_files[i], 16, MPI_CHAR, 0, 0, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
            // printf("Process %d received filename \"%s\" for index %d\n",
            // rank, local_csv_files[i], i); fflush(stdout);

            // Receive file size
            MPI_Recv(&local_csv_lines[i], 1, MPI_INT, 0, 0, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
    ...
    local_weather_array =
        malloc(sizeof(struct weather_data*) * data_sizes[rank]);

    // printf("START GETTING WEATHER DATA FOR FILES ON PROCESS %d\n", rank);
    // fflush(stdout);

    for (int i = 0; i < data_sizes[rank]; i++) {
        // printf("GETTING WEATHER DATA FOR FILE %s (LENGTH %d) ON PROCESS %d AT
        // INDEX %d\n", local_csv_files[i], local_csv_lines[i], rank, i);
        // fflush(stdout);
        local_weather_array[i] =
            get_data_array(local_csv_files[i], local_csv_lines[i]);
    }
...
#pragma omp parallel for
    for (int i = 0; i < data_sizes[rank]; i++) {
        ...
        for (int j = 0; j < local_csv_lines[i]; j++) {
            if (strcmp(tmp[j].name, "TMAX") == 0) local_temp_maxes[i]++;
            if (strcmp(tmp[j].name, "TMIN") == 0) local_temp_mins[i]++;
        }
    }
    ...
    for (int k = 0; k < data_sizes[rank]; k++) {
        ...
        for (int i = 0; i < local_csv_lines[k]; i++) {
            if (strcmp(tmp[i].name, "TMAX") == 0) {
                ...
                day = atoi(cday);
                if (month != 1)
                    local_t_x[local_count_x] =
                        (double)days_in_month[month - 2] + day;
                else
    ...
    // rank, local_output_array_size, num_processes); fflush(stdout);
    MPI_Gather(&local_output_array_size, 1, MPI_INT, local_output_array_sizes,
               1, MPI_INT, 0, MPI_COMM_WORLD);

    if (rank == 0) {
        // for (int i = 0; i < num_processes; i++) {
        //    printf("PROCESS %d: OUTPUT ARRAY SIZE OF PROCESS %d WAS RECEIVED
        //    AS %d\n", rank, i, local_output_array_sizes[i]); fflush(stdout);
        //}
        ...
        for (int i = 0; i < num_processes; i++) {
            ...
            if (i == 0) {
                ...
                local_output_array_displacements[i] =
                    local_output_array_displacements[i - 1] +
                    local_output_array_sizes[i - 1];
            }
        }

        // for (int i = 0; i < num_processes; i++) {
        //    printf("SIZE FOR ARRAY ON PROCESS %d IS %d\n", i,
        //    local_output_array_sizes[i]); printf("DISPLACEMENT FOR ARRAY ON
        //    PROCESS %d IS %d\n", i, local_output_array_displacements[i]);
        //    fflush(stdout);
        //}

        // printf("PROCESS %d: FINAL OUTPUT ARRAY SIZE IS %d\n", rank,
        // output_array_size); fflush(stdout);
    ...
    // printf("PROCESS %d: SENDING LOCAL OUTPUT ARRAY T X AND NUM_PROCESSES
    // %d\n", rank, num_processes); fflush(stdout);
    MPI_Gatherv(local_t_x, local_output_array_size, MPI_DOUBLE, t_x,
                local_output_array_sizes, local_output_array_displacements,
                MPI_DOUBLE, 0, MPI_COMM_WORLD);

    // gather t y to single array
    // printf("PROCESS %d: SENDING LOCAL OUTPUT ARRAY T Y AND NUM_PROCESSES
    // %d\n", rank, num_processes); fflush(stdout);
    MPI_Gatherv(local_t_y, local_output_array_size, MPI_DOUBLE, t_y,
                local_output_array_sizes, local_output_array_displacements,
                MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        // Takes in two arrays for points, the # of elements, and the order and
        // stores it in the coefficients pointer return 0 -> we good return -1
        // -> no good
...
// Will need to change this up soon to accept a month as well and then pick from
// the correct month ^ this will require coefficients to also change
